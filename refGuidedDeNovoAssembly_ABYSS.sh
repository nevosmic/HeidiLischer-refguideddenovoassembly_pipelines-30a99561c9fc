...
progAbyss=${progPath}/abyss/bin/abyss
...
#######################################################
  ...
  do
    ...
    cd ${unassFolder}/k${j}
    printf "#"'!'"/bin/bash\n" > runAbyss_k${j}.sh
    printf "${progAbyss}-pe k=${j} name=Unass_${j} lib=\"${abyssLib}\" ${abyssPaired} se=${bowtieFailUnpairMerged}" >> runAbyss_k${j}.sh
...
####################################################### 
  ...
  printf "#"'!'"/bin/bash\n" > runAbyss_k${kmer}.sh
  printf "${progAbyss}-pe k=${kmer} name=Unass_${kmer} lib=\"${abyssLib}\" ${abyssPaired} se=${supercontFailUnpairMerged}" >> runAbyss_k${kmer}.sh
